...
import { defaultSettings, money, today } from "./constants";
...
export function buildReceipt(receiptNo, sales, items, fallbackRep, settings = defaultSettings) {
  ...
  return {
    ...
    shopName: settings.shop_name || defaultSettings.shop_name,
    welcomeNote: settings.welcome_note || defaultSettings.welcome_note,
    thankYouNote: settings.thank_you_note || defaultSettings.thank_you_note,
    items: rows.map((sale) => ({
      itemId: sale.Item_ID,
...
export function printReceipt(receipt) {
  const html = `
    <html>
      ...
      <body>
        <div class="paper">
          <h1>${receipt.shopName || defaultSettings.shop_name}</h1>
          <p>Premium Auto & Industrial Parts</p>
          <p class="welcome">${receipt.welcomeNote || defaultSettings.welcome_note}</p>
          <div class="meta">
          ...
          <p class="thanks">${receipt.thankYouNote || defaultSettings.thank_you_note}</p>
        </div>
