async function request(action, payload = {}) {
  const response = await fetch("/api/sheets", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action, ...payload }),
  });

  const contentType = response.headers.get("content-type") || "";
  if (!contentType.includes("application/json")) {
    throw new Error("The server returned a web page instead of data. Please check the app server and API setup.");
  }

  const result = await response.json();
  if (!result.ok) {
    throw new Error(result.error || "Request failed");
  }
  return result;
}

export async function fetchDatabase() {
  const result = await request("getDashboard");
  return result.data;
}

export async function analyzeSupplyScan(payload) {
  const response = await fetch("/api/ai-supply", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });

  const result = await response.json();
  if (!result.ok) {
    throw new Error(result.error || "Supply scan failed");
  }
  return result;
}

export async function checkConnection() {
  return request("healthCheck");
}

export async function loginUser(payload) {
  return request("login", { payload });
}

export async function updateSettings(payload) {
  return request("updateSettings", { payload });
}

export async function addCategory(payload) {
  return request("addCategory", { payload });
}

export async function updateCategory(payload) {
  return request("updateCategory", { payload });
}

export async function addItem(payload) {
  return request("addItem", { payload });
}

export async function updateItem(payload) {
  return request("updateItem", { payload });
}

export async function addSupplier(payload) {
  return request("addSupplier", { payload });
}

export async function updateSupplier(payload) {
  return request("updateSupplier", { payload });
}

export async function addSalesRep(payload) {
  return request("addSalesRep", { payload });
}

export async function updateSalesRep(payload) {
  return request("updateSalesRep", { payload });
}

export async function addStock(payload) {
  return request("addStock", { payload });
}

export async function addSale(payload) {
  return request("addSale", { payload });
}

export async function addBasketSale(payload) {
  return request("addBasketSale", { payload });
}

export async function addExpense(payload) {
  return request("addExpense", { payload });
}
